...
class NotificationConfig(BaseModel):
    ...
    settings: Dict[str, Any] = Field(default_factory=dict, description="Type-specific settings")
...
class ConfigLoader:
        ...
        try:
            with open(path, 'r', encoding='utf-8') as f:
                ...
                if not isinstance(content, dict):
                    raise ValueError(f"Configuration must be a YAML dictionary, got {type(content)}")
                return content
    ...
    def _load_and_validate(self) -> None:
        ...
        try:
            ...
            error_msg = f"Configuration validation failed with {error_count} error(s):\n"
